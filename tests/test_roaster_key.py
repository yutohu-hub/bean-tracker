...
実データで実際に起きていた。
  ...
  丸山珈琲 と 堀口珈琲 が同じ店として扱われ、堀口珈琲のページに
  丸山珈琲の Instagram（@maruyama_coffee）が出ていた。突き合わせ用の
  名前が、どちらも空文字になっていたため（英数字以外を捨てていた）。

  豆の付け替えも同じ理屈で起きる。この2店は巡回で商品が取れていないので
  実際に動いた豆は無かったが、取れるようになれば同じように混ざる。
