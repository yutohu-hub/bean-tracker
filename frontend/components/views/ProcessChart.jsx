"use client";
import { FS, INK, GRAY } from "../lib/theme";
...
export function ProcessChart({ cur = "JPY", onProcess }) {
  ...
  // 内容量が取れていない豆は 0 になるので、平均の分母からも外す（数えると平均が薄まる）
  const g = {};
  for (const b of beans) {
    const k = processKey(b.process);
    const gr = (g[k] = g[k] || { n: 0, sum: 0, priced: 0 });
    gr.n++;
    if (hasPer100(b)) { gr.sum += per100JPY(b); gr.priced++; }
  }
  const rows = ORDER.filter((k) => g[k]).map((k) => ({ k, ...PROC[k], n: g[k].n, avg: g[k].priced ? g[k].sum / g[k].priced : 0, jp: JP[k] }));
  ...
  return (
    <div style={{ marginTop: 30, borderTop: `2px solid ${INK}`, paddingTop: 16 }}>
      <div style={{ fontFamily: "ui-monospace, monospace", fontSize: FS.meta, letterSpacing: "0.15em", color: GRAY }}>BY PROCESS</div>
      <div style={{ fontSize: FS.lead, fontWeight: 800, marginTop: 4 }}>精製方法ごとの内訳</div>
      <div style={{ fontSize: FS.meta, color: GRAY, marginTop: 3, lineHeight: 1.7 }}>
            ...
            <button key={r.k} onClick={() => onProcess && onProcess(r.k)}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
                <span style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: FS.body, fontWeight: 700, color: INK, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.label}</span>
                  <span style={{ fontSize: FS.meta, color: GRAY, flexShrink: 0 }}>{r.jp}</span>
                </span>
                <span style={{ display: "flex", alignItems: "baseline", gap: 6, flexShrink: 0 }}>
                  <span style={{ fontFamily: "ui-monospace, monospace", fontSize: FS.meta, color: INK }}>{r.n}<span style={{ fontSize: FS.meta, color: GRAY }}>銘柄 / {share.toFixed(0)}%</span></span>
                  <span style={{ fontSize: FS.body, color: GRAY }}>›</span>
                </span>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
                <span style={{ fontSize: FS.meta, color: GRAY }}>平均 {fmt(r.avg)} / 100g</span>
                <span style={{ fontSize: FS.meta, color: GRAY }}>ページを開く ›</span>
              </div>
            </button>
          );
        })}
      ...
      <div style={{ fontSize: FS.meta, color: GRAY, marginTop: 12, lineHeight: 1.6 }}>
