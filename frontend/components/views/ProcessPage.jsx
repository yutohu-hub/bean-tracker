...
export function ProcessPage({ pkey = "washed", onOpen, onRoaster, onBack, onProcess, cur = "JPY" }) {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontSize: FS.body, color: GRAY }}>← 味わいマップに戻る</button>
          ...
          return (
            <button key={k} onClick={() => onProcess && onProcess(k)}
              style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 999, cursor: "pointer",
                border: `1px solid ${on ? PROC[k].bg : LINE}`, background: on ? PROC[k].bg : "transparent", color: on ? "#fff" : INK, fontSize: FS.meta, fontWeight: on ? 700 : 400 }}>
              <span style={{ width: 8, height: 8, borderRadius: 3, background: on ? "#fff" : PROC[k].bg }} />
              {PROC[k].label}<span style={{ fontSize: FS.meta, opacity: 0.85 }}>{counts[k]}</span>
            </button>
          );
        })}
      </div>
      ...
      <div style={{ marginTop: 6, padding: "16px 18px", borderRadius: 14, background: st.bg, color: st.accent }}>
        <div style={{ fontFamily: "ui-monospace, monospace", fontSize: FS.meta, letterSpacing: "0.15em", opacity: 0.85 }}>PROCESS</div>
        <div style={{ fontSize: FS.title, fontWeight: 800, marginTop: 4 }}>{st.label} <span style={{ fontSize: FS.body, fontWeight: 700, opacity: 0.85 }}>{JP[key]}</span></div>
        <div style={{ fontSize: FS.body, marginTop: 8, lineHeight: 1.8, opacity: 0.95 }}>{DESC[key]}</div>
        <div style={{ display: "flex", gap: 20, marginTop: 12 }}>
          <div><div style={{ fontFamily: "ui-monospace, monospace", fontSize: FS.head, fontWeight: 800 }}>{list.length}</div><div style={{ fontSize: FS.meta, opacity: 0.85 }}>いま買える銘柄</div></div>
          <div><div style={{ fontFamily: "ui-monospace, monospace", fontSize: FS.head, fontWeight: 800 }}>{fmt(avg)}</div><div style={{ fontSize: FS.meta, opacity: 0.85 }}>平均 / 100g</div></div>
        </div>
      </div>
        ...
        <div style={{ display: "flex", border: `1px solid ${INK}`, borderRadius: 8, overflow: "hidden" }}>
          {[["list", "☰ 一覧"], ["map", "🗺 味わいマップ"]].map(([m, l]) => (
            <button key={m} onClick={() => setMode(m)}
              style={{ padding: "7px 12px", border: "none", cursor: "pointer", fontSize: FS.body, fontWeight: 700, background: mode === m ? INK : PAPER, color: mode === m ? PAPER : INK }}>{l}</button>
          ))}
        </div>
        {mode === "list" && (
      ...
      {mode === "map" ? (
        ...
        <>
          ...
          {list.length === 0 && <div style={{ textAlign: "center", color: GRAY, fontSize: FS.body, padding: "50px 0" }}>この精製の在庫はいまありません。</div>}
        </>
      )}
    </div>
  );
