    ...
    <div style={{ minHeight: "100vh", background: PAPER, color: INK, fontFamily: `"Hiragino Kaku Gothic ProN", "Hiragino Sans", "Noto Sans JP", sans-serif` }}>
      <header style={{ borderBottom: `2px solid ${INK}` }}>
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "14px 20px" }}>
          <Link href="/" style={{ fontSize: 12, color: GRAY, textDecoration: "none" }}>← BEAN TRACKER にもどる</Link>
          <div style={{ fontWeight: 800, fontSize: 18, letterSpacing: "0.04em", marginTop: 8 }}>{title}</div>
          {updated && <div style={{ fontFamily: "ui-monospace, monospace", fontSize: 10, color: GRAY, marginTop: 4 }}>最終更新: {updated}</div>}
        </div>
      </header>
      <main style={{ maxWidth: 720, margin: "0 auto", padding: "22px 20px 70px", fontSize: 13.5, lineHeight: 1.9 }}>
        {children}
        <nav style={{ marginTop: 40, borderTop: `1px solid ${LINE}`, paddingTop: 16, display: "flex", flexWrap: "wrap", gap: 16 }}>
          {pages.filter(([, l]) => l !== title).map(([href, label]) => (
            <Link key={href} href={href} style={{ fontSize: 12, color: GRAY }}>{label}</Link>
          ))}
        </nav>
      </main>
    </div>
...
export const H = ({ children }) => (
  <h2 style={{ fontSize: 15, fontWeight: 800, margin: "26px 0 8px", paddingBottom: 4, borderBottom: `1px solid ${LINE}` }}>{children}</h2>
);
